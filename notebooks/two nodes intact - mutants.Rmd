---
title: "Andrea_2-node intact mutants"
date: 2020-12-10
output:
  html_document:
    toc: true
...
brc12 <- read.csv("../data/Two nodes hormones/brc1,2_SAPL_2_node.csv") 

d14 <- read.csv("../data/Two nodes hormones/d14_SAPL_2_node.csv") 

pintr <- read.csv("../data/Two nodes hormones/pin3,4,7_SAPL_2_node.csv")


data_raw <- rbind(brc12, d14, pintr) %>% mutate(assay="2-nodes with intact apices")
...
data$genotype <- gsub("brc1-2","brc1", data$genotype)
data$genotype <- gsub("d14","d14\n", data$genotype)
data$genotype <- gsub("brc1","brc1\n", data$genotype)
data$genotype <- gsub("pin3,4,7","pin3,4,7\n", data$genotype)
...
data <- data %>% mutate(genotype = factor(genotype, levels = c("d14\n SAPL:icals3m", "brc1\n SAPL:icals3m", "pin3,4,7\n SAPL:icals3m"))) %>% mutate(assay="2-nodes with intact apices")
...
outcome.nodes =
  data_raw %>%
  select(sample_id, assay, genotype, rep, treatment, start_date, bud_location, day_08) %>%
...
data_new$genotype <- gsub("brc1-2","brc1", data_new$genotype)
data_new$genotype <- gsub("d14","d14\n", data_new$genotype)
data_new$genotype <- gsub("brc1","brc1\n", data_new$genotype)
data_new$genotype <- gsub("pin3,4,7","pin3,4,7\n", data_new$genotype)
...
data_new <- data_new %>% mutate(genotype = factor(genotype, levels = c("d14\n SAPL:icals3m", "brc1\n SAPL:icals3m", "pin3,4,7\n SAPL:icals3m"))) %>% mutate(assay="2-nodes with intact apices")

# to plot mean bud length and SE we need to do some more rearranging

#calculate mean and ds
stats_new <- data_new %>% 
  group_by(assay, genotype, treatment, status, rep, day) %>% 
  dplyr::summarise(mean = mean(bud_length, na.rm = TRUE),
                   sd = sd(bud_length, na.rm = TRUE))

#calculate n 
stats2_new <- data_new %>% 
  group_by(assay, genotype, treatment, status, day) %>% tally()

# calculate SE - such a nightmare
stats3_new <- merge(stats_new,stats2_new,by=c("genotype", "assay", "treatment", "status", "day")) %>% mutate(SE = sd/sqrt(n))

plot2 <- ggplot(stats3_new, aes(x=day, y=mean, colour=treatment)) + 
geom_vline(data=filter(stats3_new, genotype=="d14\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="d14\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="d14\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=7), size=2, colour="coral", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="d14\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="royalblue", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="brc1\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="brc1\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="brc1\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="brc1\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=5), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="pin3,4,7\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(stats3_new, genotype=="pin3,4,7\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
...
data_new %>% group_by(genotype, day, status) %>% do(tidy(wilcox.test(bud_length~treatment, data=.)))%>% ungroup() %>%
...
metrics_summary <- data_new %>% 
  nest(data = c(-sample_id, - assay, -genotype, -treatment, -status, -start_date, - assay)) %>% 
...
# all 6 but all the brc ones at 5 

metrics_summary %>%  select(sample_id, genotype, treatment, status, breakpoint_day) %>% drop_na() %>%
...
x$genotype <- gsub("brc1-2","brc1", x$genotype)
x$genotype <- gsub("d14","d14\n", x$genotype)
x$genotype <- gsub("brc1","brc1\n", x$genotype)
x$genotype <- gsub("pin3,4,7","pin3,4,7\n", x$genotype)
x$treatment <- gsub("CK", "CK (0.1uM)", x$treatment)
x$treatment <- gsub(".EST", " +\nEST (10uM)", x$treatment)


x <- x %>% ungroup() %>%  mutate(genotype = factor(genotype, levels = c("d14\n SAPL:icals3m", "brc1\n SAPL:icals3m", "pin3,4,7\n SAPL:icals3m")))
...
x <- metrics_summary %>%
 drop_na() %>% select(assay, genotype, treatment, sample_id, status)

# from 338 to 198 samples. 140 discarded
...
metrics_summary <- data_new %>% 
  nest(data = c(-sample_id, -genotype, -assay, -treatment, -status, -start_date)) %>% 
  # Run the one_node_metrics function on each individual growth curve
  mutate(segmented_data = map(data, ~ one_node_metrics(.x$day, .x$bud_length))) %>%
  unnest(cols = c(segmented_data))
...
# 1782 samples from #3042. Each dropped bud would have 9 rows. 140x9 = 1260 makes sense
...
geom_vline(data=filter(a, genotype=="d14\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="d14\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="d14\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=7), size=2, colour="coral", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="d14\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="royalblue", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="brc1\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="brc1\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="brc1\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="brc1\n SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=5), size=2, colour="steelblue2", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="pin3,4,7\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) +
  geom_vline(data=filter(a, genotype=="pin3,4,7\n SAPL:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
  #geom_rect(data=filter(metrics_summary, genotype=="d14\n SAPL:icals3m" & status=="Longer bud"), aes(xmin = 4.5, xmax = 9.5, ymin = -Inf, ymax = Inf), fill = "pink", alpha = 0.01) + 
  #geom_rect(data=filter(metrics_summary, genotype=="d14\n SAPL:icals3m" & status=="Shorter bud"), aes(xmin = 7.5, xmax = 9.5, ymin = -Inf, ymax = Inf), fill = "pink", alpha = 0.01) +
  #geom_rect(data=filter(metrics_summary, genotype=="brc1\n SAPL:icals3m" & status=="Longer bud"), aes(xmin = 6.5, xmax = 9.5, ymin = -Inf, ymax = Inf), fill = "pink", alpha = 0.01) +
  #geom_rect(data=filter(metrics_summary, genotype=="brc1\n SAPL:icals3m" & status=="Shorter bud"), aes(xmin = 8.5, xmax = 9.5, ymin = -Inf, ymax = Inf), fill = "pink", alpha = 0.01) +
