...
Col <- read.csv("../data/Two nodes decap/Col-0_2_nodes_decap.csv")

SAPL <- read.csv("../data/Two nodes decap/SAPL_2_node_decap.csv")

CALS8 <- read.csv("../data/Two nodes decap/CALS8_2_node_decap.csv")

data_raw <- rbind(Col, SAPL, CALS8)
...
data_raw <- data_raw %>% 
  rename(bud_location = bud,
         ...
         sample_id = sample,
         start_date = date) %>% 
  mutate(assay="2-nodes with decapitated apices")
...
data$treatment <- gsub("EST", "EST\n(10uM)", data$treatment)

data$day <- as.numeric(data$day)

# graphical ordering

data <- data %>% mutate(genotype = factor(genotype, levels = c("Col-0", "SAPL:icals3m", "CALS8:icals3m"))) %>% mutate(assay="2-nodes with decapitated apices")
...
plot1 <- ggplot(data, aes(x = day, y = bud_length, colour= treatment)) +
  ...
  scale_fill_manual(values = c("orange1","steelblue2")) +
  scale_colour_manual(values = c("orange1","steelblue2"))+
...
plot8 <- data %>% 
  ...
  scale_fill_manual(values = c("black","orange1","steelblue2")) +
  scale_colour_manual(values = c("black","orange1","steelblue2"))+
...
# the code is a bit clanky potentially

outcome.nodes =
  data_raw %>%
  select(sample_id, assay, genotype, rep, treatment, bud_location, day_08) %>%
...
data_new$treatment <- gsub("EST", "EST\n(10uM)", data_new$treatment)

data_new$day <- as.numeric(data_new$day)

data_new <- data_new %>% mutate(genotype = factor(genotype, levels = c("Col-0", "SAPL:icals3m", "CALS8:icals3m"))) %>% mutate(assay="2-nodes with decapitated apices")

# to plot mean bud length and SE we need to do some more rearranging

#calculate mean and ds
stats_new <- data_new %>% 
  group_by(genotype, treatment, status, day) %>% 
  dplyr::summarise(mean = mean(bud_length, na.rm = TRUE),
                   sd = sd(bud_length, na.rm = TRUE))

#calculate n 
stats2_new <- data_new %>% 
  group_by( genotype, treatment, status, day) %>% tally()

# calculate SE - such a nightmare
stats3_new <- merge(stats_new,stats2_new,by=c("genotype", "treatment", "status", "day")) %>% mutate(SE = sd/sqrt(n)) %>% 
  mutate(assay="2-nodes with decapitated apices") 
...
geom_vline(data=filter(stats3_new, genotype=="SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) + geom_vline(data=filter(stats3_new, genotype=="SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=4), size=2, colour="steelblue2", alpha=0.5) + 
    geom_vline(data=filter(stats3_new, genotype=="CALS8:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) + geom_vline(data=filter(stats3_new, genotype=="CALS8:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
...
metrics_summary <- data_new %>% 
  nest(data = c(-sample_id, -genotype, -treatment, -status, -start_date, - assay)) %>% 
...
plot3 <- metrics_summary %>% 
  ...
  scale_fill_manual(values = c("orange1","steelblue2", "black")) +
  scale_colour_manual(values = c("orange1","steelblue2", "black"))+ 
  theme(legend.position = "none") +
  theme(plot.title = element_text(hjust=.5, size = 10),
...
# all 6 but SAPL EST shorter CALS8 DMSO longer 5 


metrics_summary %>% select(sample_id, genotype, treatment, status, breakpoint_day) %>% 
  group_by(genotype, status) %>% 
  summarise(test=list(ks.test(breakpoint_day[treatment=="DMSO"], breakpoint_day[treatment=="EST\n(10uM)"]))) %>%
...
x$treatment <- gsub("EST", "EST (10uM)", x$treatment)
...
x <- metrics_summary %>%
 drop_na() %>% select(genotype, treatment, sample_id, status)

# from 322 to 272 samples. 50 discarded
...
# 2448 samples from #2898. Each dropped bud would have 9 rows. 50x9 = 450 makes sense
...
geom_vline(data=filter(a, genotype=="SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) + geom_vline(data=filter(a, genotype=="SAPL:icals3m" & status=="Shorter bud"), aes(xintercept=4), size=2, colour="steelblue2", alpha=0.5) + 
    geom_vline(data=filter(a, genotype=="CALS8:icals3m" & status=="Longer bud"), aes(xintercept=5), size=2, colour="orange1", alpha=0.5) + geom_vline(data=filter(a, genotype=="CALS8:icals3m" & status=="Longer bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) +
geom_vline(data=filter(a, genotype=="CALS8:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="orange1", alpha=0.5) + geom_vline(data=filter(a, genotype=="CALS8:icals3m" & status=="Shorter bud"), aes(xintercept=6), size=2, colour="steelblue2", alpha=0.5) + 
  #geom_rect(data=filter(metrics_summary, genotype=="SAPL:icals3m" & status=="Longer bud"), aes(xmin = 5.5, xmax = 9.5, ymin = -Inf, ymax = Inf),
                   #fill = "pink", alpha = 0.01)  +
  #geom_rect(data=filter(metrics_summary, genotype=="SAPL:icals3m" & status=="Shorter bud"), aes(xmin = 7.5, xmax = 9.5, ymin = -Inf, ymax = Inf),
                   #fill = "pink", alpha = 0.01) + geom_boxplot(alpha=0.5) +
    theme(plot.title = element_text(hjust=.5, size = 10),
        axis.title.x = element_text(size = 16), 
        axis.title.y = element_text(size = 16), 
        axis.text.x = element_text(size = 16), 
